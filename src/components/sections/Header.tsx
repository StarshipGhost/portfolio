...
const MobileHeader = ({open, closeMenu, scroll}: {open: boolean, closeMenu: () => void, scroll: (index: number) => void}) => {
    ...
    return (
        <ul className={clsx(baseClass, activeClass)}>
            ...
            <li className='text-white text-sm text-center font-medium bg-linear-to-r from-cyan-500 hover:from-cyan-600 to-purple-500 hover:to-purple-600 rounded-md p-2 mb-4 cursor-pointer' onClick={closeMenu}><a href="CV_Saad_Kalyati_FR.pdf" target="_blank">French Resume</a></li>
            <li className='text-white text-sm text-center font-medium bg-linear-to-r from-cyan-500 hover:from-cyan-600 to-purple-500 hover:to-purple-600 rounded-md p-2 cursor-pointer' onClick={closeMenu}><a href="CV_Saad_Kalyati_EN.pdf" target="_blank">English Resume</a></li>
        </ul>
    )
}
...
const Header = ({scroll} : { scroll : (index: number) => void}) => {
    ...
    const headerRef : RefObject<HTMLElement | null> = useRef<HTMLElement | null>(null);
    useOutsideClick(headerRef, openMenu, closeMenu);
    ...
    return (
        <header ref={headerRef} className="sticky top-0 z-1">
            <div className="flex justify-between items-center bg-black lg:px-[5vw] px-4 py-2 border-b border-solid border-cyan-900/70">
                ...
                <div className="hidden lg:block">
                    ...
                    <div className={ clsx(popoverBaseClass, popoverActiveClass) }>
                       ...
                       <a className={ `text-cyan-500 text-sm font-medium border border-box border-cyan-900 hover:border-cyan-500 hover:bg-cyan-500/10 ${hovered ? 'block pointer-events-auto cursor-pointer' : 'hidden pointer-events-none cursor-default'} p-4`} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} href="CV_Saad_Kalyati_EN.pdf" target="_blank">English</a>
                    </div>
                </div>
            </div>
