...
const EducationCard = ({education : {icon, school, program, specialization, year}} : {education: EducationCardProps}) => {
    return (
        <div className="bg-cyan-500/10 backdrop-blur-3xl border border-solid border-cyan-900 hover:border-cyan-500 flex flex-col gap-4 rounded-lg p-4">
            <div className='flex justify-between'>
                <div className='flex gap-4'>
                    ...
                    <div className='space-y-2'>
                        ...
                        <p className='text-cyan-500 font-medium'>{program}</p> 
                    </div>
                </div> 
            ...
            <p className='max-w-4xl text-neutral-400'>{specialization}</p>
        </div>
    )
}
...
const EducationCards = ({education} : {education: EducationCardProps[]}) => {
    return (
        <div className='w-full space-y-8'>
            {education.map(edu => <EducationCard education={edu}/>)};
        </div>
    ) 
}
...
const Education = () => {
    return (
        <section className='max-w-7xl mx-auto flex flex-col gap-4 items-center px-4 py-20'>
            <GraduationCapIcon/>
            <BracketWrapper color={'text-cyan-500'} className="text-3xl lg:text-4xl text-white font-bold">Education</BracketWrapper>
            <p className="text-neutral-400 text-md lg:text-lg tracking-wide text-center mb-8">My academic journey and qualifications</p>
